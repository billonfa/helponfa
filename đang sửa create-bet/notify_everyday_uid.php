<?php
    ...
    FROM customers
    WHERE telegram REGEXP '[^0-9]'
    AND DATE(ngay_tim_ve) = CURDATE() - INTERVAL 1 DAY
    AND (tinh_trang = 2 OR tinh_trang = 3 OR tinh_trang_mg = 2 OR tinh_trang_mg = 3)
    GROUP BY nhan_vien";
    ...
    if ($result->num_rows > 0) {
        ...
        while ($row = $result->fetch_assoc()) {
            $id_nv = $row['nhan_vien'];
            $soluong = $row['count'];
            $sql_name_nv = "SELECT username FROM users WHERE id = '$id_nv'";
