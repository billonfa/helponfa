...
const change_money = (money) => {
    money.addEventListener("input", function () {
        var userInput = money.value;
        var formatted = userInput.replace(/,/g, ''); // Xóa tất cả dấu phẩy có sẵn
        formatted = formatted.replace(/\D/g, ""); // Xóa tất cả ký tự không phải số
        formatted = parseFloat(formatted);
        if (!isNaN(formatted)) { // Kiểm tra nếu giá trị là một số hợp lệ
            formatted = formatted.toLocaleString('vi-VN'); // Định dạng số với dấu phẩy
            formatted = formatted.replace(/\./g, ','); // Thay thế dấu chấm thành dấu phẩy
            money.value = formatted;
        } else { // Nếu giá trị không hợp lệ
            money.value = ""; // Xóa giá trị trong ô input
        }
    });
}
...
top_right.append(div_battery, div_orders, div_time)
...
option1_status.textContent = 'Rút tiền thành công'
option1_status.value = 'Rút tiền thành công'
...
select_status.append(option1_status, option2_status, option3_status, option4_status)
...
change_money(input_price)
input_price.value = formatMoney(getRandomInt(1, 20) * 100)
div_price.append(label_price, input_price)

const bank = document.createElement('div')
const label_bank = document.createElement('label')
label_bank.textContent = "Ngân hàng rút tiền"
label_bank.appendChild(data_auto.cloneNode(true))
const select_bank = document.createElement('select')
select_bank.classList.add('select')
// const acb_bank = document.createElement('option')
// acb_bank.textContent = "ACB"
// const bidv_bank = document.createElement('option')
// bidv_bank.textContent = "BIDV"
const agribank_bank = document.createElement('option')
agribank_bank.textContent = "AGRI BANK"
// const msb_bank = document.createElement('option')
// msb_bank.textContent = "MSB"
const sacombank_bank = document.createElement('option')
sacombank_bank.textContent = "SACOM BANK"
const techcombank_bank = document.createElement('option')
techcombank_bank.textContent = "TECHCOM BANK"
const vietcombank_bank = document.createElement('option')
vietcombank_bank.textContent = "VIETCOM BANK"
const vietinbank_bank = document.createElement('option')
vietinbank_bank.textContent = "VIETIN BANK"
const ocb_bank = document.createElement('option')
ocb_bank.textContent = 'OCBBANK'
select_bank.append(agribank_bank, sacombank_bank, techcombank_bank, vietcombank_bank, vietinbank_bank, ocb_bank)
bank.append(label_bank, select_bank)

left_mid.append(h4_don1, div_detail_time, div_status, div_price, bank)
...
option1_status_d2.textContent = 'Rút tiền thành công'
option1_status_d2.value = 'Rút tiền thành công'
...
select_status_d2.append(option1_status_d2, option2_status_d2, option3_status_d2, option4_status_d2)
...
change_money(input_price_d2)
input_price_d2.value = formatMoney(getRandomInt(1, 20) * 100)
div_price_d2.append(label_price.cloneNode(true), input_price_d2)

const bank_d2 = document.createElement('div')
const select_bank_d2 = document.createElement('select')
select_bank_d2.classList.add('select')
select_bank_d2.append(agribank_bank.cloneNode(true), sacombank_bank.cloneNode(true), techcombank_bank.cloneNode(true), vietcombank_bank.cloneNode(true), vietinbank_bank.cloneNode(true))
bank_d2.append(label_bank.cloneNode(true), select_bank_d2)
...
option1_status_d3.textContent = 'Rút tiền thành công'
option1_status_d3.value = 'Rút tiền thành công'
...
select_status_d3.append(option1_status_d3, option2_status_d3, option3_status_d3, option4_status_d3)
...
change_money(input_price_d3)
input_price_d3.value = formatMoney(getRandomInt(1, 20) * 100)
div_price_d3.append(label_price.cloneNode(true), input_price_d3)

const bank_d3 = document.createElement('div')
const select_bank_d3 = document.createElement('select')
select_bank_d3.classList.add('select', 'bank_d3')
select_bank_d3.append(agribank_bank.cloneNode(true), sacombank_bank.cloneNode(true), techcombank_bank.cloneNode(true), vietcombank_bank.cloneNode(true), vietinbank_bank.cloneNode(true))
bank_d3.append(label_bank.cloneNode(true), select_bank_d3)

const div_total_money_d3 = document.createElement('div')
div_total_money_d3.classList.add('total_money')
const label_total_money_d3 = document.createElement('label')
label_total_money_d3.classList.add('form-label')
label_total_money_d3.textContent = 'Tổng số tiền 3 đơn'
const input_total_money_d3 = document.createElement('input')
input_total_money_d3.classList.add('form-control')
change_money(input_total_money_d3)
div_total_money_d3.append(label_total_money_d3, input_total_money_d3)
...
div_button_import_data.append(button_import_data)
...
select_orders.addEventListener('change', function () {
    ...
    if (select_orders.value == 1) {
        ...
        right_mid.append(h4_don2, div_detail_time_d2, div_status_d2, div_price_d2, bank_d2)
        left_bot.append(h4_don3, div_detail_time_d3, div_status_d3, div_price_d3, bank_d3, div_total_money_d3)
...
function import_data1(div_import) {
    ...
    photo_don1.src = 'photo-withdrawal/bongvip_1don.png'
    ...
    div_import.append(clock, battery, text_battery, wifi)
    ...
    const bank_import = document.createElement('div')
    bank_import.classList.add('bank')
    bank_import.textContent = select_bank.value
    ...
    const trang_thai = document.createElement('div')
    trang_thai.classList.add('trang_thai')
    trang_thai.textContent = "Trạng thái"
    ...
    trade_code.textContent = '1261' + getRandomInt(100000, 999999)
    ...
    // Tùy chỉnh màu chữ Trạng thái
    if(select_status.value == 'Rút tiền thành công') {
        status_import.style.color = '#35be42'
    }
    else if(select_status.value == 'Đang xử lý') {
        status_import.style.color = 'black'
    }
    else if(select_status.value == 'Rút tiền thất bại') {
        status_import.style.color = 'red'
    }
    else if(select_status.value == 'Kiểm duyệt thành công') {
        status_import.style.color = '#35be42'
    }

    const total_withdrawal = document.createElement('div')
    total_withdrawal.classList.add('total_withdrawal')
    total_withdrawal.innerHTML = (input_price.value) + '.00'

    div_import.append(time, bank_import, detail_time, trang_thai, withdrawal, trade_code, status_import, total_withdrawal)
}

function import_data2(div_import) {
    div_import.innerHTML = ''
    const photo_don1 = document.createElement('img')
    photo_don1.src = 'photo-withdrawal/bongvip_2don.png'
    photo_don1.setAttribute('id', 'photo_show')
    photo_don1.style.width = '800px'
    div_import.appendChild(photo_don1)
    ...
    text_url.classList.add('text-url')
    text_url.textContent = arr_url[getRandomInt(0, 15)]
    
    const text_battery = document.createElement('div')
    text_battery.classList.add('text_battery_v2')
    text_battery.textContent = `${input_battery.value}`
    ...
    div_import.append(clock, battery, text_battery, wifi, text_url)
    // End Phụ trợ của điện thoại

    const time = document.createElement('div')
    time.classList.add('time_v2')
    time.textContent = input_time.value

    // Thông tin đơn cược 1
    const bank_import = document.createElement('div')
    bank_import.classList.add('bank_v2')
    bank_import.textContent = select_bank.value
    ...
    const trang_thai = document.createElement('div')
    trang_thai.classList.add('trang_thai_v2')
    trang_thai.textContent = "Trạng thái"
    ...
    const trade_code = document.createElement('div')
    const number_trade_code = '1261' + getRandomInt(100000, 999999)
    ...
    div_import.append(time, bank_import, detail_time, trang_thai, withdrawal, trade_code, status_import)
    // End thông tin đơn cược 1

    // Thông tin đơn cược 2
    const bank_import_d2 = document.createElement('div')
    bank_import_d2.classList.add('bank_v2_d2')
    bank_import_d2.textContent = select_bank_d2.value
    ...
    const trang_thai_d2 = document.createElement('div')
    trang_thai_d2.classList.add('trang_thai_v2_d2')
    trang_thai_d2.textContent = "Trạng thái"
    ...
    trade_code_d2.textContent = number_trade_code - getRandomInt(100, 10000)
    ...
    if(select_status_d2.value == 'Rút tiền thành công') {
        status_import_d2.style.color = '#35be42'
    }
    else if(select_status_d2.value == 'Đang xử lý') {
        status_import_d2.style.color = 'black'
    }
    else if(select_status_d2.value == 'Rút tiền thất bại') {
        status_import_d2.style.color = 'red'
    }
    else if(select_status_d2.value == 'Kiểm duyệt thành công') {
        status_import_d2.style.color = '#35be42'
    }
    ...
    const total_withdrawal_d2 = document.createElement('div')
    total_withdrawal_d2.classList.add('total_withdrawal_v2')
    total_withdrawal_d2.innerHTML = total_price + '.00'
    // End Thông tin đơn cược 2
    div_import.append(bank_import_d2, detail_time_d2, trang_thai_d2, withdrawal_d2, trade_code_d2, status_import_d2, total_withdrawal_d2)
}

function import_data3(div_import) {
    div_import.innerHTML = ''
    const photo_don1 = document.createElement('img')
    photo_don1.src = 'photo-withdrawal/bongvip_3don.png'
    photo_don1.setAttribute('id', 'photo_show')
    photo_don1.style.width = '800px'
    div_import.appendChild(photo_don1)
    ...
    const total_withdrawal_d2 = document.createElement('div')
    total_withdrawal_d2.classList.add('total_withdrawal_v2')
    total_withdrawal_d2.innerHTML = (input_price_d2.value) + '.00'
    // End Thông tin đơn cược 2
    div_import.append(bank_import_d2, detail_time_d2, trang_thai_d2, withdrawal_d2, trade_code_d2, status_import_d2)

    // Thông tin đơn cược 3
    const bank_import_d3 = document.createElement('div')
    bank_import_d3.classList.add('bank_v2_d3')
    bank_import_d3.textContent = select_bank_d3.value
    ...
    const trang_thai_d3 = document.createElement('div')
    trang_thai_d3.classList.add('trang_thai_v2_d3')
    trang_thai_d3.textContent = "Trạng thái"
    if(select_status_d3.value == 'Rút tiền thành công') {
        status_import_d3.textContent = "Rút tiền"
        status_import_d3.style.color = '#35be42'
    }
    else if(select_status_d3.value == 'Đang xử lý') {
        status_import_d3.textContent = "Đang xử lý"
        status_import_d3.style.color = 'black'
    }
    else if(select_status_d3.value == 'Rút tiền thất bại') {
        status_import_d3.textContent = "Rút tiền"
        status_import_d3.style.color = 'red'
    }
    else if(select_status_d3.value == 'Kiểm duyệt thành công') {
        status_import_d3.textContent = "Kiểm duyệt"
        status_import_d3.style.color = '#35be42'
    }
    ...
    const total_withdrawal_d3 = document.createElement('div')
    total_withdrawal_d3.classList.add('total_withdrawal_v2')
    console.log(input_total_money_d3.value)
    total_withdrawal_d3.innerHTML = input_total_money_d3.value + '.00'
    
    // End Thông tin đơn cược 3
    div_import.append(bank_import_d3, detail_time_d3, trang_thai_d3, withdrawal_d3, trade_code_d3, status_import_d3, total_withdrawal_d3)
